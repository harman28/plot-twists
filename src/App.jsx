...
function Pill({ children, color, small }) {
  return <span style={{ fontSize: small?"11px":"12px", letterSpacing:"0.1em", textTransform:"uppercase", padding:"2px 7px", borderRadius:"2px", background:color+"22", color, fontWeight:600, whiteSpace:"nowrap" }}>{children}</span>;
}
...
function TabBar({ active, onChange }) {
  return (
    <div style={{ display:"flex", alignItems:"center", gap:0, height:"100%" }}>
      {TABS.map(t => (
        <button key={t.id} onClick={() => onChange(t.id)} style={{ ...F, height:"100%", padding:"0 20px", background:"transparent", border:"none", borderBottom: active===t.id ? "2px solid #5B4FCE":"2px solid transparent", color: active===t.id ? "#18100A":"#6B5B78", fontSize:"13px", letterSpacing:"0.1em", textTransform:"uppercase", cursor:"pointer", transition:"color 0.15s" }}>
          {t.label}
        </button>
      ))}
    </div>
...
function BottomTabBar({ active, onChange }) {
  return (
    <div style={{ position:"fixed", bottom:0, left:0, right:0, height:"56px", background:"#F3EFFE", borderTop:"1px solid rgba(91,79,206,0.15)", display:"flex", zIndex:200, backdropFilter:"blur(8px)" }}>
      {TABS.map(t => (
        <button key={t.id} onClick={() => onChange(t.id)}
          style={{ flex:1, background:"transparent", border:"none", borderTop: active===t.id ? "2px solid #5B4FCE" : "2px solid transparent", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:"2px", cursor:"pointer", color: active===t.id ? "#18100A" : "#6B5B78", transition:"color 0.15s", padding:0 }}>
          <span style={{ fontSize:"18px", lineHeight:1 }}>{t.icon}</span>
          <span style={{ ...F, fontSize:"11px", letterSpacing:"0.08em", textTransform:"uppercase" }}>{t.label}</span>
        </button>
      ))}
    </div>
  );
...
function NotesModal({ item, notes, onSave, onClose }) {
  ...
  const c = COLOR[item.theme] || "#6B5B78";
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(10,30,20,0.65)", zIndex:200, display:"flex", alignItems:"center", justifyContent:"center", padding:"20px" }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ ...F, background:"#FEFEFD", border:"1px solid rgba(91,79,206,0.20)", borderRadius:"6px", width:"100%", maxWidth:"540px", maxHeight:"90vh", display:"flex", flexDirection:"column", overflow:"hidden" }}>
        <div style={{ padding:"16px 18px 12px", borderBottom:"1px solid rgba(91,79,206,0.25)", display:"flex", alignItems:"flex-start", gap:"10px" }}>
          <div style={{ flex:1 }}>
            <Pill color={c}>{item.theme}</Pill>
            <div style={{ fontSize:"15px", color:"#18100A", marginTop:"7px", lineHeight:1.4, fontWeight:500 }}>{item.title}</div>
            <div style={{ fontSize:"13px", color:"#18100A", fontStyle:"italic", marginTop:"3px" }}>{item.source} · {item.published}</div>
          </div>
          <button onClick={onClose} style={{ background:"transparent", border:"none", color:"#18100A", cursor:"pointer", fontSize:"20px", flexShrink:0 }}>×</button>
        </div>
        <div style={{ flex:1, overflowY:"auto", padding:"16px 18px", display:"flex", flexDirection:"column", gap:"14px" }}>
          <div>
            <label style={{ fontSize:"12px", color:"#6B5B78", letterSpacing:"0.12em", textTransform:"uppercase", display:"block", marginBottom:"6px" }}>Key quote</label>
            <textarea value={quo} onChange={e => setQuo(e.target.value)} placeholder="Paste a passage worth keeping…" rows={3}
              style={{ ...F, width:"100%", background:"rgba(243,239,254,0.5)", border:"1px solid rgba(91,79,206,0.15)", borderRadius:"3px", color:"#18100A", fontSize:"14px", padding:"9px 11px", resize:"vertical", outline:"none", boxSizing:"border-box", lineHeight:1.6, fontStyle:"italic" }} />
          </div>
          <div>
            <label style={{ fontSize:"12px", color:"#6B5B78", letterSpacing:"0.12em", textTransform:"uppercase", display:"block", marginBottom:"6px" }}>What is being argued?</label>
            <textarea value={arg} onChange={e => setArg(e.target.value)} placeholder="Summarise the core argument…" rows={4}
              style={{ ...F, width:"100%", background:"rgba(243,239,254,0.7)", border:"1px solid rgba(91,79,206,0.15)", borderRadius:"3px", color:"#18100A", fontSize:"14px", padding:"9px 11px", resize:"vertical", outline:"none", boxSizing:"border-box", lineHeight:1.6 }} />
          </div>
          <div>
            <label style={{ fontSize:"12px", color:"#18100A", letterSpacing:"0.12em", textTransform:"uppercase", display:"block", marginBottom:"6px" }}>My thoughts / response</label>
        ...
        <div style={{ padding:"12px 18px", borderTop:"1px solid rgba(74,96,48,0.14)", display:"flex", gap:"8px", justifyContent:"flex-end" }}>
          {hasContent && <button onClick={() => { onSave(item.url, null); onClose(); }} style={{ ...F, background:"transparent", border:"1px solid rgba(91,79,206,0.30)", color:"#5B4FCE", padding:"6px 14px", borderRadius:"3px", fontSize:"12px", letterSpacing:"0.1em", textTransform:"uppercase", cursor:"pointer" }}>Delete note</button>}
          <button onClick={onClose} style={{ ...F, background:"transparent", border:"1px solid rgba(91,79,206,0.25)", color:"#6B5B78", padding:"6px 14px", borderRadius:"3px", fontSize:"12px", letterSpacing:"0.1em", textTransform:"uppercase", cursor:"pointer" }}>Cancel</button>
          <button onClick={() => { onSave(item.url, { argument:arg, thoughts:tho, quote:quo }); onClose(); }} style={{ ...F, background:"transparent", border:"1px solid #5B4FCE", color:"#5B4FCE", padding:"6px 14px", borderRadius:"3px", fontSize:"12px", letterSpacing:"0.1em", textTransform:"uppercase", cursor:"pointer" }}>Save note</button>
        </div>
      </div>
    </div>
  );
...
function DispatchItem({ item, show, idx, readItems, onToggleRead, notes, onOpenNote }) {
  ...
  return (
    <div style={{ opacity:show?(isRead?0.42:1):0, transform:show?"none":"translateY(8px)", transition:`opacity 0.35s ease ${idx*0.06}s, transform 0.35s ease ${idx*0.06}s`, borderBottom:"1px solid rgba(91,79,206,0.20)", paddingBottom:"18px", marginBottom:"18px" }}>
      <div style={{ display:"flex", gap:"7px", flexWrap:"wrap", alignItems:"center", marginBottom:"7px" }}>
        <Pill color={c}>{item.theme}</Pill>
        {item.type==="foundational" && <Pill color="#5B4FCE">Foundational</Pill>}
        {item.custom && <Pill color="#A78BFA">Custom</Pill>}
        <span style={{ fontSize:"12px", color:"#18100A" }}>{item.published}</span>
        <span style={{ fontSize:"12px", color:"#18100A", marginLeft:"auto" }}>{item.readingMinutes} min</span>
      </div>
      <a href={item.url} target="_blank" rel="noopener noreferrer"
        style={{ fontSize:"16px", color:isRead?"#18100A":"#18100A", textDecoration:isRead?"line-through":"none", textDecorationColor:"#444", display:"block", lineHeight:"1.4", marginBottom:"8px", fontWeight:500, transition:"color 0.15s", ...F }}
        onMouseEnter={e => { if(!isRead) e.currentTarget.style.color=c; }}
        onMouseLeave={e => { if(!isRead) e.currentTarget.style.color="#18100A"; }}>
        {item.title} <span style={{ opacity:0.3, fontSize:"13px" }}>↗</span>
      </a>
      <div style={{ display:"flex", gap:"7px", flexWrap:"wrap", alignItems:"center" }}>
        <span style={{ fontSize:"13px", color:"#18100A", fontStyle:"italic" }}>{item.source}</span>
        <span style={{ color:"#6B5B78" }}>·</span>
        {item.keywords.map((kw,j) => <span key={j} style={{ fontSize:"12px", color:"#D97706", background:"rgba(217,119,6,0.12)", border:"1px solid rgba(217,119,6,0.20)", padding:"1px 6px", borderRadius:"2px" }}>#{kw}</span>)}
        <div style={{ marginLeft:"auto", display:"flex", gap:"5px" }}>
          <button onClick={() => onOpenNote(item)}
            style={{ ...F, background: hasNote?"rgba(219,39,119,0.10)":"rgba(243,239,254,0.7)", border: hasNote?"1px solid rgba(219,39,119,0.30)":"1px solid rgba(91,79,206,0.20)", borderRadius:"3px", padding:"2px 8px", cursor:"pointer", fontSize:"12px", letterSpacing:"0.07em", textTransform:"uppercase", color: hasNote?"#DB2777":"#6B5B78" }}>
            {hasNote ? "✎ Note" : "Note"}
          </button>
          <button onClick={() => onToggleRead(item.url)}
            style={{ ...F, background: isRead?"rgba(22,163,74,0.15)":"rgba(243,239,254,0.7)", border: isRead?"1px solid rgba(22,163,74,0.50)":"1px solid rgba(91,79,206,0.20)", borderRadius:"3px", padding:"2px 8px", cursor:"pointer", fontSize:"12px", letterSpacing:"0.07em", textTransform:"uppercase", color: isRead?"#16A34A":"#6B5B78" }}>
            {isRead ? "✓ Read" : "To Read"}
          </button>
        </div>
      </div>
  ...
  return (
    <div style={{ ...F, maxWidth:"680px", margin:"0 auto", padding:"0 24px 80px" }}>
      ...
      <div style={{ paddingTop:"32px", paddingBottom:"20px" }}>
        <div style={{ display:"flex", alignItems:"center", gap:"10px", marginBottom:"20px", flexWrap:"wrap" }}>
          ...
          <button onClick={() => { setPickerVal(date); setShowPicker(v=>!v); }} style={{ ...F, background:"transparent", border:"none", cursor:"pointer", padding:"0 2px" }}>
            <span style={{ fontSize:"15px", color: isToday?"#5B4FCE":"#18100A", fontStyle:"italic", borderBottom:"1px dashed currentColor", opacity:0.85 }}>{fmtDate(date)}</span>
          </button>
          <button onClick={() => { if(!isToday) load(nextDay(date)); }} style={{ ...NAV, opacity:isToday?0.2:1, cursor:isToday?"default":"pointer" }}>→</button>
          {!isToday && <button onClick={() => load(todayStr())} style={{ ...F, fontSize:"11px", letterSpacing:"0.12em", textTransform:"uppercase", background:"transparent", border:"1px solid rgba(91,79,206,0.30)", color:"#5B4FCE", padding:"3px 9px", borderRadius:"2px", cursor:"pointer" }}>Today</button>}
          {issue && <span style={{ fontSize:"13px", color:"#18100A", marginLeft:"auto", fontStyle:"italic" }}>{readCount}/{allItems.length} read</span>}
        </div>

        {showPicker && (
          <div style={{ marginBottom:"16px", background:"rgba(243,239,254,0.7)", border:"1px solid rgba(91,79,206,0.15)", borderRadius:"4px", padding:"11px 14px", display:"flex", alignItems:"center", gap:"10px", flexWrap:"wrap" }}>
            <span style={{ fontSize:"12px", color:"#18100A", textTransform:"uppercase", letterSpacing:"0.1em" }}>Jump to</span>
            <input type="date" value={pickerVal} max={todayStr()} onChange={e=>setPickerVal(e.target.value)}
              style={{ background:"transparent", border:"1px solid rgba(91,79,206,0.25)", borderRadius:"3px", color:"#18100A", padding:"4px 8px", fontSize:"14px", ...F }} />
            <button onClick={() => { if(pickerVal){load(pickerVal);setShowPicker(false);} }}
              style={{ ...F, background:"transparent", border:"1px solid #5B4FCE", color:"#5B4FCE", padding:"4px 12px", borderRadius:"3px", cursor:"pointer", fontSize:"12px", letterSpacing:"0.1em", textTransform:"uppercase" }}>Go</button>
            <button onClick={() => setShowPicker(false)} style={{ background:"transparent", border:"none", color:"#18100A", cursor:"pointer", fontSize:"18px", marginLeft:"auto" }}>×</button>
          </div>
        )}

        {/* Theme pills */}
        <div style={{ display:"flex", gap:"5px", flexWrap:"wrap", marginBottom:"20px" }}>
          {THEMES.map(t => <span key={t.name} style={{ fontSize:"11px", letterSpacing:"0.08em", textTransform:"uppercase", padding:"3px 7px", borderRadius:"2px", border:"1px solid "+t.color+"40", color:t.color+"99" }}>{t.name}</span>)}
        </div>

        {!issue && (
          <button onClick={() => load(date)} style={{ ...F, padding:"11px 26px", background:"transparent", border:"1px solid #5B4FCE", color:"#5B4FCE", fontSize:"13px", letterSpacing:"0.15em", textTransform:"uppercase", cursor:"pointer", borderRadius:"3px" }}>
      ...
      {issue && (
        <>
          {/* Progress bar */}
          <div style={{ height:"2px", background:"rgba(91,79,206,0.25)", borderRadius:"1px", marginBottom:"26px", overflow:"hidden", opacity:show?1:0, transition:"opacity 0.4s ease 0.2s" }}>
          ...
          <div style={{ marginBottom:"40px" }}>
            <div style={{ display:"flex", alignItems:"center", gap:"12px", marginBottom:"20px", paddingBottom:"10px", borderBottom:"1px solid rgba(91,79,206,0.15)" }}>
              <span style={{ fontSize:"12px", color:"#5B4FCE", letterSpacing:"0.18em", textTransform:"uppercase" }}>I — In the Field</span>
              <span style={{ fontSize:"13px", color:"#6B5B78", fontStyle:"italic" }}>Essays · Reports · Journalism</span>
            </div>
          ...
          <div>
            <div style={{ display:"flex", alignItems:"center", gap:"12px", marginBottom:"20px", paddingBottom:"10px", borderBottom:"1px solid rgba(0,191,255,0.22)" }}>
              <span style={{ fontSize:"12px", color:"#5B4FCE", letterSpacing:"0.18em", textTransform:"uppercase" }}>II — The Literature</span>
              <span style={{ fontSize:"13px", color:"#18100A", fontStyle:"italic" }}>Papers · Foundational works</span>
            </div>
          ...
          <div style={{ marginTop:"40px", paddingTop:"14px", borderTop:"1px solid rgba(155,98,48,0.09)", display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:"8px", opacity:show?1:0, transition:"opacity 0.4s ease 0.6s" }}>
            <span style={{ fontSize:"12px", color:"#6B5B78", fontStyle:"italic" }}>{pool.length} items in pool · rotates daily</span>
            <button onClick={() => load(date)} style={{ ...F, background:"transparent", border:"1px solid rgba(91,79,206,0.20)", color:"#6B5B78", fontSize:"12px", letterSpacing:"0.1em", textTransform:"uppercase", padding:"4px 11px", cursor:"pointer", borderRadius:"2px" }}>Reshuffle</button>
          </div>
        </>
      )}
...
function AddSourceView({ pool, onAdd, onDelete, hiddenIds, allBuiltin, onHide, onRestore }) {
  ...
  const inputStyle = { ...F, width:"100%", background:"rgba(243,239,254,0.7)", border:"1px solid rgba(91,79,206,0.20)", borderRadius:"3px", color:"#18100A", fontSize:"14px", padding:"8px 10px", outline:"none", boxSizing:"border-box" };
  const labelStyle = { fontSize:"12px", color:"#6B5B78", letterSpacing:"0.1em", textTransform:"uppercase", display:"block", marginBottom:"5px" };
  const errStyle   = { fontSize:"12px", color:"#5B4FCE", marginTop:"3px" };

  return (
    <div style={{ ...F, maxWidth:"680px", margin:"0 auto", padding:"24px 24px 80px" }}>
      <div style={{ marginBottom:"28px" }}>
        <h2 style={{ fontSize:"20px", color:"#18100A", fontWeight:400, margin:"0 0 6px", letterSpacing:"-0.01em" }}>Sow a Source</h2>
        <p style={{ fontSize:"14px", color:"#6B5B78", fontStyle:"italic", margin:0 }}>New sources enter the pool immediately and will appear in future plots and the grove.</p>
      </div>
      ...
      <div style={{ marginBottom:"20px" }}>
        ...
        <div style={{ display:"flex", gap:"6px", flexWrap:"wrap", marginBottom:"8px" }}>
          {form.keywords.map((kw,i) => (
            <span key={i} style={{ fontSize:"13px", color:"#5B4FCE", background:"rgba(91,79,206,0.10)", border:"1px solid rgba(91,79,206,0.30)", padding:"2px 8px", borderRadius:"2px", display:"flex", alignItems:"center", gap:"5px" }}>
              #{kw}
              <button onClick={() => set("keywords",form.keywords.filter((_,j)=>j!==i))} style={{ background:"none", border:"none", color:"#5B4FCE", cursor:"pointer", fontSize:"13px", padding:0, lineHeight:1 }}>×</button>
            </span>
          ))}
        </div>
        <div style={{ display:"flex", gap:"8px" }}>
          ...
          <button onClick={() => addKw(kwInput)} style={{ ...F, background:"transparent", border:"1px solid rgba(91,79,206,0.25)", color:"#6B5B78", padding:"0 14px", borderRadius:"3px", cursor:"pointer", fontSize:"13px", whiteSpace:"nowrap" }}>Add</button>
        </div>
        <div style={{ fontSize:"12px", color:"#6B5B78", marginTop:"5px" }}>Pick from existing or type new ones. Shared keywords create edges in the Grove.</div>
        {errors.keywords && <div style={errStyle}>{errors.keywords}</div>}
      </div>

      <button onClick={submit} style={{ ...F, padding:"10px 28px", background:"transparent", border:"1px solid #5B4FCE", color:"#5B4FCE", fontSize:"13px", letterSpacing:"0.15em", textTransform:"uppercase", cursor:"pointer", borderRadius:"3px", marginBottom:"8px" }}>
        Add to Pool
      </button>
      {saved && <span style={{ fontSize:"13px", color:"#16A34A", marginLeft:"12px" }}>✓ Added successfully</span>}

      {/* Custom items list */}
      {customItems.length > 0 && (
        <div style={{ marginTop:"36px" }}>
          <div style={{ fontSize:"12px", color:"#6B5B78", letterSpacing:"0.12em", textTransform:"uppercase", marginBottom:"14px" }}>Your custom sources ({customItems.length})</div>
          {customItems.map(item => (
            <div key={item.id} style={{ display:"flex", alignItems:"flex-start", gap:"12px", borderBottom:"1px solid rgba(91,79,206,0.20)", paddingBottom:"12px", marginBottom:"12px" }}>
              <div style={{ flex:1 }}>
                <div style={{ display:"flex", gap:"5px", marginBottom:"4px" }}>
                  <Pill color={COLOR[item.theme]||"#6B5B78"}>{item.theme}</Pill>
                </div>
                <a href={item.url} target="_blank" rel="noopener noreferrer" style={{ fontSize:"14px", color:"#18100A", textDecoration:"none", display:"block", marginBottom:"3px" }}>{item.title} ↗</a>
                <div style={{ fontSize:"13px", color:"#18100A", fontStyle:"italic" }}>{item.source} · {item.published} · {item.readingMinutes} min</div>
              </div>
              <button onClick={() => onDelete(item.id)} style={{ ...F, background:"transparent", border:"1px solid rgba(91,79,206,0.20)", color:"#5B4FCE", padding:"3px 9px", borderRadius:"3px", cursor:"pointer", fontSize:"11px", letterSpacing:"0.1em", textTransform:"uppercase", flexShrink:0 }}>Remove</button>
            </div>
          ))}
        </div>
      )}
      ...
      {hiddenIds?.size > 0 && (
        <div style={{ marginTop:"36px" }}>
          <div style={{ fontSize:"12px", color:"#6B5B78", letterSpacing:"0.12em", textTransform:"uppercase", marginBottom:"14px" }}>Hidden sources ({hiddenIds.size}) — click Restore to bring back</div>
          {(allBuiltin || []).filter(item => hiddenIds.has(item.id)).map(item => (
            <div key={item.id} style={{ display:"flex", alignItems:"flex-start", gap:"12px", borderBottom:"1px solid rgba(91,79,206,0.20)", paddingBottom:"10px", marginBottom:"10px", opacity:0.6 }}>
              <div style={{ flex:1 }}>
                <Pill color={COLOR[item.theme]||"#6B5B78"}>{item.theme}</Pill>
                <div style={{ fontSize:"13px", color:"#18100A", marginTop:"4px" }}>{item.title}</div>
                <div style={{ fontSize:"13px", color:"#18100A", fontStyle:"italic", marginTop:"2px" }}>{item.source} · {item.published}</div>
              </div>
              <button onClick={() => onRestore(item.id)} style={{ ...F, background:"transparent", border:"1px solid rgba(91,79,206,0.25)", color:"#6B5B78", padding:"3px 9px", borderRadius:"3px", cursor:"pointer", fontSize:"11px", letterSpacing:"0.1em", textTransform:"uppercase", flexShrink:0 }}>Restore</button>
            </div>
          ))}
        </div>
      )}
...
function GardenSidebar({ node, onClose, readItems, onToggleRead, notes, onOpenNote, connectedTitles, onNavigate, publicMode, onRemove, paths, onSavePath, orgs, orgLinks, onSaveOrgLink, onDeleteOrgLink }) {
  if(!node) return null;
  const c = COLOR[node.theme]||"#6B5B78";
  ...
  return (
    <div style={{ ...F, position:"absolute", top:0, right:0, bottom:0, width:"290px", background:"rgba(237,232,218,1.0)", borderLeft:"1px solid rgba(91,79,206,0.20)", zIndex:30, display:"flex", flexDirection:"column" }}>
      <div style={{ padding:"14px 16px 12px", borderBottom:"1px solid rgba(91,79,206,0.25)", display:"flex", alignItems:"flex-start", justifyContent:"space-between", gap:"8px" }}>
        <div style={{ display:"flex", gap:"5px", flexWrap:"wrap" }}>
          <Pill color={c}>{node.theme}</Pill>
          {node.type==="foundational" && <Pill color="#5B4FCE">Foundational</Pill>}
          {node.custom && <Pill color="#A78BFA">Custom</Pill>}
        </div>
        <button onClick={onClose} style={{ background:"transparent", border:"none", color:"#18100A", cursor:"pointer", fontSize:"20px", lineHeight:1, flexShrink:0 }}>×</button>
      </div>
      <div style={{ flex:1, overflowY:"auto", padding:"14px 16px" }}>
        <div style={{ fontSize:"16px", color:"#18100A", lineHeight:"1.4", marginBottom:"8px", fontWeight:500 }}>{node.title}</div>
        <div style={{ fontSize:"13px", color:"#18100A", fontStyle:"italic", marginBottom:"3px" }}>{node.source}</div>
        <div style={{ fontSize:"12px", color:"#18100A", marginBottom:"12px" }}>{node.published} · {node.readingMinutes} min</div>
        <div style={{ display:"flex", gap:"5px", flexWrap:"wrap", marginBottom:"14px" }}>
          {node.keywords.map((kw,i) => <span key={i} style={{ fontSize:"12px", color:c, background:c+"14", border:"1px solid "+c+"30", padding:"2px 7px", borderRadius:"2px" }}>#{kw}</span>)}
        </div>
        ...
        {hasNote && (
          <div style={{ background:"rgba(155,98,48,0.06)", border:"1px solid rgba(155,98,48,0.18)", borderRadius:"3px", padding:"10px 12px", marginBottom:"12px", display:"flex", flexDirection:"column", gap:"8px" }}>
            {noteData.quote && <div style={{ fontSize:"13px", color:"#18100A", lineHeight:1.6, fontStyle:"italic", borderLeft:"2px solid rgba(219,39,119,0.35)", paddingLeft:"8px" }}><span style={{ fontSize:"11px", color:"#5B4FCE", textTransform:"uppercase", letterSpacing:"0.1em", display:"block", marginBottom:"3px", fontStyle:"normal" }}>Quote</span>{publicMode ? noteData.quote : (noteData.quote.slice(0,140)+(noteData.quote.length>140?"…":""))}</div>}
            {noteData.argument && <div style={{ fontSize:"13px", color:"#18100A", lineHeight:1.5 }}><span style={{ fontSize:"11px", color:"#5B4FCE", textTransform:"uppercase", letterSpacing:"0.1em", display:"block", marginBottom:"3px" }}>Argument</span>{publicMode ? noteData.argument : (noteData.argument.slice(0,120)+(noteData.argument.length>120?"…":""))}</div>}
            {noteData.thoughts && <div style={{ fontSize:"13px", color:"#18100A", lineHeight:1.5 }}><span style={{ fontSize:"11px", color:"#5B4FCE", textTransform:"uppercase", letterSpacing:"0.1em", display:"block", marginBottom:"3px" }}>My thoughts</span>{publicMode ? noteData.thoughts : (noteData.thoughts.slice(0,120)+(noteData.thoughts.length>120?"…":""))}</div>}
          </div>
        )}

        {connectedTitles.length > 0 && (
          <div>
            <div style={{ fontSize:"11px", color:"#6B5B78", letterSpacing:"0.1em", textTransform:"uppercase", marginBottom:"7px" }}>
              Connected via shared keywords — <span style={{ fontStyle:"italic" }}>click to navigate</span>
            </div>
            {connectedTitles.slice(0,8).map((ct,i) => {
              const cc = COLOR[ct.theme] || "#6B5B78";
              return (
                ...
                >
                  <div style={{ fontSize:"11px", color:cc, marginBottom:"2px", letterSpacing:"0.04em" }}>
                    #{ct.keywords.join(" · #")}
                  </div>
                  <div style={{ fontSize:"13px", color:"#18100A", lineHeight:1.4, ...F }}>
                    {ct.title.length>52 ? ct.title.slice(0,52)+"…" : ct.title}
                  </div>
                  <div style={{ fontSize:"12px", color:"#6B5B78", fontStyle:"italic", marginTop:"1px" }}>
                    {ct.source}
                  </div>
                </button>
              );
            ...
            {connectedTitles.length>8 && (
              <div style={{ fontSize:"12px", color:"#6B5B78", paddingLeft:"9px", marginTop:"4px", fontStyle:"italic" }}>
        ...
        {/* Reading paths */}
        {!publicMode && paths && (
          <div style={{ marginTop:"14px", paddingTop:"12px", borderTop:"1px solid rgba(91,79,206,0.08)" }}>
            <div style={{ fontSize:"11px", color:"#6B5B78", letterSpacing:"0.1em", textTransform:"uppercase", marginBottom:"7px" }}>Trails</div>
            {paths.filter(p => (p.item_ids||[]).includes(node.id)).map(p => (
              <div key={p.id} style={{ display:"flex", alignItems:"center", gap:"5px", marginBottom:"4px", background:p.color+"12", border:"1px solid "+p.color+"30", borderRadius:"3px", padding:"4px 7px" }}>
                ...
                <span style={{ ...F, fontSize:"12px", color:"#18100A", flex:1 }}>{p.name}</span>
                <span style={{ fontSize:"11px", color:p.color }}>#{(p.item_ids||[]).indexOf(node.id)+1}</span>
                <button onClick={() => onSavePath({...p, item_ids:(p.item_ids||[]).filter(id=>id!==node.id)})}
                  style={{ background:"none", border:"none", cursor:"pointer", color:p.color, fontSize:"14px", padding:0, lineHeight:1, opacity:0.7 }}>×</button>
              </div>
            ))}
        ...
        {!publicMode && orgs && orgs.length > 0 && (
          <div style={{ marginTop:"12px", paddingTop:"12px", borderTop:"1px solid rgba(91,79,206,0.08)" }}>
            <div style={{ fontSize:"11px", color:"#6B5B78", letterSpacing:"0.1em", textTransform:"uppercase", marginBottom:"7px" }}>Organisations</div>
            {orgs.filter(org => orgLinks.some(l => l.org_id === org.id && l.item_id === node.id)).map(org => {
              const color = STANCE_COLORS[org.stance] || "#6B5B78";
              return (
                <div key={org.id} style={{ display:"flex", alignItems:"center", gap:"5px", marginBottom:"4px", background:color+"10", border:"1px solid "+color+"25", borderRadius:"3px", padding:"4px 7px" }}>
                  ...
                  <span style={{ ...F, fontSize:"12px", color:"#18100A", flex:1 }}>{org.name}</span>
                  <span style={{ fontSize:"11px", color }}>{org.stance.split(" / ")[0]}</span>
                  <button onClick={() => onDeleteOrgLink(org.id, node.id)} style={{ background:"none", border:"none", cursor:"pointer", color, fontSize:"14px", padding:0, lineHeight:1, opacity:0.7 }}>×</button>
                </div>
              );
            })}
      ...
      <div style={{ padding:"12px 14px", borderTop:"1px solid rgba(91,79,206,0.25)", display:"flex", gap:"5px", flexWrap:"wrap" }}>
        <a href={node.url} target="_blank" rel="noopener noreferrer" style={{ flex:1, textAlign:"center", padding:"7px 4px", background:"transparent", border:"1px solid "+c, color:c, borderRadius:"3px", fontSize:"12px", letterSpacing:"0.1em", textTransform:"uppercase", textDecoration:"none", ...F }}>Read ↗</a>
        {!publicMode && <button onClick={() => onOpenNote(node)} style={{ flex:1, padding:"7px 4px", background:hasNote?"rgba(219,39,119,0.08)":"transparent", border:hasNote?"1px solid rgba(219,39,119,0.30)":"1px solid rgba(91,79,206,0.20)", color:hasNote?"#DB2777":"#18100A", borderRadius:"3px", fontSize:"12px", letterSpacing:"0.1em", textTransform:"uppercase", cursor:"pointer", ...F }}>
          {hasNote?"✎ Note":"Note"}
        </button>}
        {!publicMode && <button onClick={() => onToggleRead(node.url)} style={{ flex:1, padding:"7px 4px", background:isRead?"rgba(22,163,74,0.15)":"transparent", border:isRead?"1px solid rgba(22,163,74,0.50)":"1px solid rgba(91,79,206,0.20)", color:isRead?"#16A34A":"#18100A", borderRadius:"3px", fontSize:"12px", letterSpacing:"0.1em", textTransform:"uppercase", cursor:"pointer", ...F }}>
          {isRead?"✓ Read":"To Read"}
        </button>}
        {!publicMode && onRemove && <button onClick={() => { onRemove(node); onClose(); }} style={{ flex:1, padding:"7px 4px", background:"transparent", border:"1px solid rgba(91,79,206,0.20)", color:"#5B4FCE", borderRadius:"3px", fontSize:"12px", letterSpacing:"0.1em", textTransform:"uppercase", cursor:"pointer", ...F }}>Remove</button>}
      </div>
    </div>
  );
}
  ...
  if (creating) return (
    <div style={{ display:"flex", flexDirection:"column", gap:"5px" }}>
      <input value={newName} onChange={e => setNewName(e.target.value)} placeholder="Path name…" autoFocus
        ...
        style={{ ...F, background:"rgba(243,239,254,0.7)", border:"1px solid rgba(91,79,206,0.20)", borderRadius:"3px", color:"#18100A", fontSize:"13px", padding:"5px 8px", outline:"none" }} />
      <div style={{ display:"flex", gap:"4px", flexWrap:"wrap" }}>
        {PATH_COLORS.map(c => (
          <button key={c} onClick={() => setNewColor(c)}
            style={{ width:"16px", height:"16px", borderRadius:"50%", background:c, border: newColor===c?"2px solid #18100A":"1px solid transparent", cursor:"pointer", padding:0, flexShrink:0 }} />
        ))}
      </div>
      <div style={{ display:"flex", gap:"5px" }}>
        <button onClick={createAndAdd} style={{ ...F, flex:1, background:"transparent", border:"1px solid #5B4FCE", color:"#5B4FCE", padding:"4px", borderRadius:"3px", fontSize:"11px", letterSpacing:"0.1em", textTransform:"uppercase", cursor:"pointer" }}>Create</button>
        <button onClick={() => setCreating(false)} style={{ ...F, flex:1, background:"transparent", border:"1px solid rgba(91,79,206,0.20)", color:"#6B5B78", padding:"4px", borderRadius:"3px", fontSize:"11px", letterSpacing:"0.1em", textTransform:"uppercase", cursor:"pointer" }}>Cancel</button>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ position:"relative" }}>
      <button onClick={() => setOpen(v => !v)}
        style={{ ...F, width:"100%", background:"transparent", border:"1px solid rgba(91,79,206,0.20)", borderRadius:"3px", padding:"5px 8px", fontSize:"11px", letterSpacing:"0.1em", textTransform:"uppercase", color:"#6B5B78", cursor:"pointer", textAlign:"left" }}>
        + Add to trail {open ? "▲" : "▼"}
      </button>
      {open && (
        <div style={{ position:"absolute", bottom:"calc(100% + 4px)", left:0, right:0, background:"#F3EFFE", border:"1px solid rgba(91,79,206,0.15)", borderRadius:"3px", boxShadow:"0 -4px 12px rgba(28,43,28,0.1)", overflow:"hidden", zIndex:50 }}>
          {paths.length > 0 && available.length === 0 && (
            <div style={{ padding:"7px 10px", fontSize:"12px", color:"#6B5B78", fontStyle:"italic" }}>Already in all paths</div>
          )}
          {available.map(p => (
            <button key={p.id} onClick={() => addToPath(p)}
              style={{ ...F, display:"flex", width:"100%", textAlign:"left", background:"transparent", border:"none", borderBottom:"1px solid rgba(91,79,206,0.10)", padding:"6px 10px", cursor:"pointer", alignItems:"center", gap:"6px" }}
              onMouseEnter={e => e.currentTarget.style.background="rgba(243,239,254,0.7)"}
              onMouseLeave={e => e.currentTarget.style.background="transparent"}>
              <div style={{ width:"7px", height:"7px", borderRadius:"50%", background:p.color, flexShrink:0 }} />
              <span style={{ fontSize:"12px", color:"#18100A", flex:1 }}>{p.name}</span>
              <span style={{ fontSize:"11px", color:"#6B5B78" }}>{(p.item_ids||[]).length} items</span>
            </button>
          ))}
          <button onClick={() => { setOpen(false); setCreating(true); }}
            style={{ ...F, display:"block", width:"100%", textAlign:"left", background:"transparent", border:"none", padding:"6px 10px", cursor:"pointer", fontSize:"11px", color:"#5B4FCE", letterSpacing:"0.08em", textTransform:"uppercase" }}
            onMouseEnter={e => e.currentTarget.style.background="rgba(243,239,254,0.7)"}
            onMouseLeave={e => e.currentTarget.style.background="transparent"}>
            + New trail…
          </button>
...
function PathsPanel({ paths, pool, onSavePath, onDeletePath, onClose }) {
  ...
  return (
    <div style={{ ...F, position:"absolute", bottom:"44px", left:"10px", zIndex:40, background:"#F3EFFE", border:"1px solid rgba(91,79,206,0.20)", borderRadius:"4px", padding:"12px", width:"270px", maxHeight:"72vh", overflowY:"auto", boxShadow:"0 4px 16px rgba(43,45,66,0.1)" }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:"12px" }}>
        <span style={{ fontSize:"11px", color:"#6B5B78", letterSpacing:"0.12em", textTransform:"uppercase" }}>Trails</span>
        <button onClick={flushAndClose} style={{ background:"none", border:"none", cursor:"pointer", color:"#6B5B78", fontSize:"15px", padding:0, lineHeight:1 }}>×</button>
      </div>

      {paths.length === 0 && (
        <div style={{ fontSize:"13px", color:"#6B5B78", fontStyle:"italic" }}>No trails yet. Open any source in the grove and add it to a new trail.</div>
      )}
      ...
      {paths.map(p => {
        ...
        return (
          <div key={p.id} style={{ marginBottom:"10px", background:"rgba(243,239,254,0.5)", border:"1px solid rgba(91,79,206,0.08)", borderLeft:"3px solid "+p.color, borderRadius:"3px", padding:"8px 10px" }}>
            ...
            <div style={{ display:"flex", alignItems:"center", gap:"5px" }}>
              {isEditing ? (
                <input value={editName} onChange={e => setEditNameSynced(e.target.value)}
                  ...
                  style={{ ...F, flex:1, background:"transparent", border:"none", borderBottom:"1px solid rgba(91,79,206,0.30)", outline:"none", fontSize:"13px", color:"#18100A", padding:"1px 0", fontWeight:500 }} />
              ) : (
                <span onClick={() => { setEditingId(p.id); setEditNameSynced(p.name); setEditDescSynced(p.description||""); setEditFocus("name"); }}
                  style={{ flex:1, fontSize:"13px", color:"#18100A", cursor:"text", fontWeight:500, lineHeight:1.3 }} title="Click to rename">{p.name}</span>
              )}
              <button onClick={() => setExpanded(v => v===p.id ? null : p.id)}
                style={{ background:"none", border:"none", cursor:"pointer", color:"#6B5B78", fontSize:"12px", padding:"0 2px", lineHeight:1, flexShrink:0 }}>
                {(p.item_ids||[]).length} {isExpanded ? "▲" : "▼"}
              </button>
              <button onClick={() => onDeletePath(p.id)}
                style={{ background:"none", border:"none", cursor:"pointer", color:"#5B4FCE", fontSize:"14px", padding:0, lineHeight:1, flexShrink:0, opacity:0.6 }}>×</button>
            </div>
            ...
            {isEditing ? (
              <textarea value={editDesc} onChange={e => setEditDescSynced(e.target.value)} placeholder="Describe this trail…" rows={2}
                ...
                onBlur={() => commitEdit(p)}
                style={{ ...F, width:"100%", marginTop:"6px", background:"rgba(243,239,254,0.5)", border:"1px solid rgba(91,79,206,0.15)", borderRadius:"2px", color:"#18100A", fontSize:"12px", padding:"4px 6px", resize:"none", outline:"none", boxSizing:"border-box", lineHeight:1.5, fontStyle:"italic" }} />
            ) : (
              <div onClick={() => { setEditingId(p.id); setEditNameSynced(p.name); setEditDescSynced(p.description||""); setEditFocus("desc"); }}
                style={{ fontSize:"12px", color: p.description ? "#6B5B78" : "rgba(122,128,104,0.45)", fontStyle:"italic", marginTop:"4px", lineHeight:1.4, cursor:"text" }}>
            ...
            {isExpanded && (p.item_ids||[]).length > 0 && (
              <div style={{ marginTop:"8px", borderTop:"1px solid rgba(91,79,206,0.08)", paddingTop:"6px" }}>
                {(p.item_ids||[]).map((id, idx) => {
                  ...
                  return (
                    <div key={id} style={{ display:"flex", alignItems:"center", gap:"4px", marginBottom:"4px" }}>
                      <span style={{ fontSize:"11px", color:p.color, fontWeight:"bold", width:"14px", flexShrink:0, textAlign:"right" }}>{idx+1}</span>
                      <span style={{ flex:1, fontSize:"12px", color:"#18100A", lineHeight:1.3 }}>
                        {item.title.length>34 ? item.title.slice(0,34)+"…" : item.title}
                      </span>
                      <button onClick={() => moveItem(p, idx, idx-1)} disabled={idx===0}
                        style={{ background:"none", border:"none", cursor:idx===0?"default":"pointer", color:idx===0?"rgba(122,128,104,0.25)":"#6B5B78", fontSize:"11px", padding:0, lineHeight:1, flexShrink:0 }}>▲</button>
                      <button onClick={() => moveItem(p, idx, idx+1)} disabled={last}
                        style={{ background:"none", border:"none", cursor:last?"default":"pointer", color:last?"rgba(122,128,104,0.25)":"#6B5B78", fontSize:"11px", padding:0, lineHeight:1, flexShrink:0 }}>▼</button>
                      <button onClick={() => removeItemFromPath(p, id)}
                        style={{ background:"none", border:"none", cursor:"pointer", color:"#5B4FCE", fontSize:"13px", padding:0, lineHeight:1, flexShrink:0, opacity:0.6 }}>×</button>
                    </div>
                  );
                })}
              </div>
            ...
            {isExpanded && (p.item_ids||[]).length === 0 && (
              <div style={{ marginTop:"6px", fontSize:"12px", color:"#6B5B78", fontStyle:"italic" }}>No items yet.</div>
            )}
          </div>
        );
      })}
...
function PathsView({ paths, pool, notes }) {
  ...
  if (paths.length === 0) return (
    <div style={{ ...F, maxWidth:"680px", margin:"0 auto", padding:"80px 24px", textAlign:"center" }}>
      <div style={{ fontSize:"15px", color:"#18100A", fontStyle:"italic" }}>No trails yet.</div>
      <div style={{ fontSize:"13px", color:"#6B5B78", marginTop:"8px" }}>Open the Grove, click any source, and add it to a new trail from the sidebar.</div>
    </div>
  );
  ...
  return (
    <div style={{ display:"flex", height:"calc(100vh - 44px)", overflow:"hidden" }}>
      {/* Left: path list */}
      <div style={{ width:"210px", flexShrink:0, borderRight:"1px solid rgba(91,79,206,0.15)", overflowY:"auto", padding:"14px 0" }}>
        {paths.map(p => (
          <button key={p.id} onClick={() => setSelectedId(p.id)}
            style={{ ...F, display:"block", width:"100%", textAlign:"left", background: selected?.id===p.id ? "rgba(91,79,206,0.10)" : "transparent", border:"none", borderLeft:"3px solid "+(selected?.id===p.id ? p.color : "transparent"), padding:"10px 14px", cursor:"pointer", transition:"all 0.15s" }}>
            <div style={{ display:"flex", alignItems:"center", gap:"7px", marginBottom:"3px" }}>
              <div style={{ width:"7px", height:"7px", borderRadius:"50%", background:p.color, flexShrink:0 }} />
              <span style={{ fontSize:"13px", color:"#18100A", fontWeight: selected?.id===p.id ? 600 : 400 }}>{p.name}</span>
            </div>
            {p.description && <div style={{ fontSize:"12px", color:"#6B5B78", paddingLeft:"14px", lineHeight:1.4, marginBottom:"2px", fontStyle:"italic" }}>{p.description.length>40 ? p.description.slice(0,40)+"…" : p.description}</div>}
            <div style={{ fontSize:"12px", color:"#6B5B78", paddingLeft:"14px" }}>{(p.item_ids||[]).length} sources</div>
          </button>
        ))}
      </div>
        ...
        <div style={{ flex:1, overflowY:"auto", padding:"28px 36px 80px" }}>
          <div style={{ marginBottom:"28px", paddingBottom:"18px", borderBottom:"2px solid "+selected.color+"33" }}>
            <div style={{ display:"flex", alignItems:"center", gap:"10px", marginBottom:"8px" }}>
              ...
              <h1 style={{ ...F, fontSize:"22px", color:"#18100A", fontWeight:400, margin:0, letterSpacing:"-0.01em" }}>{selected.name}</h1>
            </div>
            {selected.description && (
              <p style={{ ...F, fontSize:"14px", color:"#6B5B78", fontStyle:"italic", margin:"0 0 0 20px", lineHeight:1.6 }}>{selected.description}</p>
            )}
          </div>

          {(selected.item_ids||[]).length === 0 && (
            <div style={{ fontSize:"13px", color:"#6B5B78", fontStyle:"italic" }}>No sources in this trail yet.</div>
          )}
            ...
            return (
              <div key={id} style={{ display:"flex", gap:"16px", marginBottom:"30px" }}>
                <div style={{ flexShrink:0, width:"26px", height:"26px", borderRadius:"50%", background:selected.color, display:"flex", alignItems:"center", justifyContent:"center", marginTop:"2px" }}>
                  <span style={{ fontSize:"12px", color:"white", fontWeight:"bold" }}>{idx+1}</span>
                </div>
                <div style={{ flex:1, borderLeft:"1px solid rgba(155,98,48,0.12)", paddingLeft:"16px" }}>
                  <div style={{ display:"flex", gap:"5px", flexWrap:"wrap", alignItems:"center", marginBottom:"7px" }}>
                    <Pill color={c}>{item.theme}</Pill>
                    {item.type==="foundational" && <Pill color="#5B4FCE">Foundational</Pill>}
                    <span style={{ fontSize:"12px", color:"#6B5B78" }}>{item.source} · {item.published}</span>
                    <span style={{ fontSize:"12px", color:"#6B5B78", marginLeft:"auto" }}>{item.readingMinutes} min</span>
                  </div>
                  <a href={item.url} target="_blank" rel="noopener noreferrer"
                    style={{ ...F, fontSize:"16px", color:"#18100A", textDecoration:"none", display:"block", lineHeight:"1.4", marginBottom:"12px", fontWeight:500 }}
                    onMouseEnter={e => e.currentTarget.style.color=c}
                    onMouseLeave={e => e.currentTarget.style.color="#18100A"}>
                    {item.title} <span style={{ opacity:0.3, fontSize:"13px" }}>↗</span>
                  </a>
                    ...
                    <div style={{ display:"flex", flexDirection:"column", gap:"10px" }}>
                      {note.quote && (
                        <div style={{ borderLeft:"2px solid "+selected.color+"55", paddingLeft:"10px" }}>
                          <div style={{ fontSize:"11px", color:selected.color, letterSpacing:"0.1em", textTransform:"uppercase", marginBottom:"3px" }}>Quote</div>
                          <div style={{ ...F, fontSize:"13px", color:"#18100A", fontStyle:"italic", lineHeight:1.7 }}>{note.quote}</div>
                        </div>
                      )}
                      {note.argument && (
                        <div>
                          <div style={{ fontSize:"11px", color:"#5B4FCE", letterSpacing:"0.1em", textTransform:"uppercase", marginBottom:"3px" }}>Argument</div>
                          <div style={{ ...F, fontSize:"13px", color:"#18100A", lineHeight:1.6 }}>{note.argument}</div>
                        </div>
                      )}
                      {note.thoughts && (
                        <div>
                          <div style={{ fontSize:"11px", color:"#7A9B6A", letterSpacing:"0.1em", textTransform:"uppercase", marginBottom:"3px" }}>My thoughts</div>
                          <div style={{ ...F, fontSize:"13px", color:"#6B5B78", lineHeight:1.6 }}>{note.thoughts}</div>
                        </div>
                      )}
                    </div>
                  )}
...
function OrgLinker({ node, orgs, orgLinks, onSaveOrgLink, onDeleteOrgLink }) {
  ...
  return (
    <div style={{ position:"relative" }}>
      ...
      {open && (
        <div style={{ position:"absolute", bottom:"calc(100% + 4px)", left:0, right:0, background:"#F3EFFE", border:"1px solid rgba(91,79,206,0.15)", borderRadius:"3px", boxShadow:"0 -4px 12px rgba(28,43,28,0.1)", overflow:"hidden", zIndex:50 }}>
          <div style={{ padding:"5px 8px", borderBottom:"1px solid rgba(91,79,206,0.10)" }}>
            <input value={searchQ} onChange={e => setSearchQ(e.target.value)} placeholder="Search organisations…" autoFocus
              style={{ ...F, width:"100%", background:"transparent", border:"none", outline:"none", fontSize:"12px", color:"#18100A", boxSizing:"border-box" }} />
          </div>
          {available.length === 0 && <div style={{ padding:"7px 10px", fontSize:"12px", color:"#6B5B78", fontStyle:"italic" }}>No organisations to link.</div>}
          {available.slice(0,6).map(org => {
            ...
            return (
              <button key={org.id} onClick={() => { onSaveOrgLink(org.id, node.id); setOpen(false); setSearchQ(""); }}
                ...
                <span style={{ fontSize:"12px", color:"#18100A", flex:1 }}>{org.name}</span>
                <span style={{ fontSize:"11px", color:"#6B5B78" }}>{org.stance.split(" / ")[0]}</span>
              </button>
            );
          })}
        </div>
...
function AddOrgModal({ org, onSave, onClose }) {
  const iStyle = { ...F, width:"100%", background:"rgba(243,239,254,0.7)", border:"1px solid rgba(91,79,206,0.20)", borderRadius:"3px", color:"#18100A", fontSize:"14px", padding:"8px 10px", outline:"none", boxSizing:"border-box" };
  const lStyle = { fontSize:"12px", color:"#6B5B78", letterSpacing:"0.1em", textTransform:"uppercase", display:"block", marginBottom:"5px" };
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(10,30,20,0.65)", zIndex:200, display:"flex", alignItems:"center", justifyContent:"center", padding:"20px" }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ ...F, background:"#FEFEFD", border:"1px solid rgba(91,79,206,0.20)", borderRadius:"6px", width:"100%", maxWidth:"460px", overflow:"hidden" }}>
        <div style={{ padding:"16px 18px 12px", borderBottom:"1px solid rgba(91,79,206,0.25)", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
          <span style={{ fontSize:"14px", color:"#18100A", fontWeight:500 }}>{org ? "Edit Organisation" : "Add Organisation"}</span>
          <button onClick={onClose} style={{ background:"transparent", border:"none", color:"#18100A", cursor:"pointer", fontSize:"20px" }}>×</button>
        </div>
        ...
        <div style={{ padding:"12px 18px", borderTop:"1px solid rgba(155,98,48,0.12)", display:"flex", gap:"8px", justifyContent:"flex-end" }}>
          ...
          <button onClick={() => { if(form.name.trim()) onSave(form); }}
            style={{ ...F, background:"transparent", border:"1px solid #5B4FCE", color:"#5B4FCE", padding:"6px 14px", borderRadius:"3px", fontSize:"12px", letterSpacing:"0.1em", textTransform:"uppercase", cursor:"pointer" }}>
            {org ? "Save" : "Add"}
          </button>
        </div>
      </div>
...
function OrgCard({ org, links, pool, isExpanded, onToggle, onEdit, onDelete, onSaveOrgLink, onDeleteOrgLink, publicMode }) {
  ...
  return (
    <div style={{ background:"rgba(243,239,254,0.5)", border:"1px solid rgba(91,79,206,0.10)", borderLeft:"3px solid "+color, borderRadius:"3px", padding:"10px 14px" }}>
      <div style={{ display:"flex", alignItems:"flex-start", gap:"8px" }}>
        <div style={{ flex:1 }}>
          <div style={{ display:"flex", alignItems:"center", gap:"8px", marginBottom:"3px" }}>
            <span style={{ ...F, fontSize:"14px", color:"#18100A", fontWeight:500 }}>{org.name}</span>
            {org.website && <a href={org.website} target="_blank" rel="noopener noreferrer" style={{ fontSize:"12px", color, textDecoration:"none" }}>↗</a>}
          </div>
          {org.description && <div style={{ ...F, fontSize:"13px", color:"#6B5B78", fontStyle:"italic", lineHeight:1.4 }}>{org.description}</div>}
        </div>
        <div style={{ display:"flex", gap:"4px", alignItems:"center", flexShrink:0 }}>
          <button onClick={onToggle}
            style={{ ...F, background:"transparent", border:"1px solid rgba(91,79,206,0.20)", borderRadius:"3px", padding:"3px 8px", cursor:"pointer", fontSize:"11px", color:"#6B5B78", letterSpacing:"0.07em", whiteSpace:"nowrap" }}>
            {links.length} {links.length===1?"article":"articles"} {isExpanded?"▲":"▼"}
          </button>
          {!publicMode && <button onClick={onEdit} style={{ background:"none", border:"none", cursor:"pointer", color:"#6B5B78", fontSize:"13px", padding:"0 2px" }}>✎</button>}
          {!publicMode && <button onClick={onDelete} style={{ background:"none", border:"none", cursor:"pointer", color:"#5B4FCE", fontSize:"14px", padding:0, opacity:0.6 }}>×</button>}
        </div>
      </div>

      {isExpanded && (
        <div style={{ marginTop:"10px", paddingTop:"10px", borderTop:"1px solid rgba(91,79,206,0.08)" }}>
          {linkedItems.map(item => (
            <div key={item.id} style={{ display:"flex", alignItems:"center", gap:"7px", marginBottom:"5px" }}>
              <Pill color={COLOR[item.theme]||"#6B5B78"} small>{item.theme}</Pill>
              <span style={{ ...F, fontSize:"13px", color:"#18100A", flex:1, lineHeight:1.3 }}>{item.title.length>52 ? item.title.slice(0,52)+"…" : item.title}</span>
              {!publicMode && <button onClick={() => onDeleteOrgLink(org.id, item.id)} style={{ background:"none", border:"none", cursor:"pointer", color:"#5B4FCE", fontSize:"13px", padding:0, opacity:0.6, flexShrink:0 }}>×</button>}
            </div>
          ))}
          {linkedItems.length === 0 && <div style={{ fontSize:"12px", color:"#6B5B78", fontStyle:"italic", marginBottom:"8px" }}>No articles linked yet.</div>}
          ...
          {!publicMode && <div style={{ marginTop:"8px", position:"relative" }}>
            <input value={searchQ} onChange={e => setSearchQ(e.target.value)} placeholder="Search articles to link…"
              ...
              style={{ ...F, width:"100%", background:"rgba(243,239,254,0.7)", border:"1px solid rgba(91,79,206,0.15)", borderRadius:"3px", color:"#18100A", fontSize:"13px", padding:"5px 8px", outline:"none", boxSizing:"border-box" }} />
            {showSearch && searchResults.length > 0 && (
              <div style={{ position:"absolute", top:"calc(100% + 2px)", left:0, right:0, background:"#F3EFFE", border:"1px solid rgba(91,79,206,0.15)", borderRadius:"3px", zIndex:50, overflow:"hidden", boxShadow:"0 4px 12px rgba(28,43,28,0.1)" }}>
                {searchResults.map(item => (
                  <button key={item.id} onMouseDown={() => { onSaveOrgLink(org.id, item.id); setSearchQ(""); }}
                    style={{ ...F, display:"block", width:"100%", textAlign:"left", background:"transparent", border:"none", borderBottom:"1px solid rgba(91,79,206,0.10)", padding:"6px 10px", cursor:"pointer" }}
                    onMouseEnter={e => e.currentTarget.style.background="rgba(243,239,254,0.7)"}
                    onMouseLeave={e => e.currentTarget.style.background="transparent"}>
                    <div style={{ fontSize:"12px", color:"#18100A" }}>{item.title.length>54 ? item.title.slice(0,54)+"…" : item.title}</div>
                    <div style={{ fontSize:"11px", color:"#6B5B78" }}>{item.source}</div>
                  </button>
                ))}
              </div>
            )}
  ...
  return (
    <div style={{ ...F, maxWidth:"800px", margin:"0 auto", padding:"28px 24px 80px" }}>
      <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", marginBottom:"28px", gap:"12px", flexWrap:"wrap" }}>
        <div>
          <h2 style={{ fontSize:"20px", color:"#18100A", fontWeight:400, margin:"0 0 5px" }}>The Field</h2>
          <p style={{ ...F, fontSize:"13px", color:"#6B5B78", fontStyle:"italic", margin:0 }}>Organisations, labs, journals, and actors shaping the discourse.</p>
        </div>
        {!publicMode && <button onClick={() => { setEditOrg(null); setShowAdd(true); }}
          style={{ ...F, background:"transparent", border:"1px solid #5B4FCE", color:"#5B4FCE", padding:"7px 16px", borderRadius:"3px", fontSize:"12px", letterSpacing:"0.1em", textTransform:"uppercase", cursor:"pointer", flexShrink:0 }}>
          + Add Organisation
        </button>}
      </div>

      {orgs.length === 0 && (
        <div style={{ textAlign:"center", padding:"60px 0", color:"#6B5B78", fontStyle:"italic", fontSize:"14px" }}>
        ...
        return (
          <div key={stance} style={{ marginBottom:"32px" }}>
            <button
              ...
              <span style={{ fontSize:"13px", color, letterSpacing:"0.12em", textTransform:"uppercase", fontWeight:600 }}>{stance}</span>
              <span style={{ fontSize:"13px", color:"#6B5B78" }}>{group.length}</span>
              <span style={{ marginLeft:"auto", fontSize:"13px", color:"#6B5B78", opacity:0.6 }}>{isCollapsed ? "▸" : "▾"}</span>
            </button>
            {!isCollapsed && (
...
function OrgSidebar({ org, orgLinks, pool, onClose, onNavigate }) {
  ...
  return (
    <div style={{ position:"absolute", top:0, right:0, width:"280px", height:"100%", background:"#F3EFFE", borderLeft:"1px solid rgba(91,79,206,0.15)", display:"flex", flexDirection:"column", zIndex:30, overflow:"hidden" }}>
      <div style={{ padding:"14px 14px 10px", borderBottom:"1px solid rgba(91,79,206,0.08)", flexShrink:0 }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:"8px" }}>
          <div style={{ display:"flex", alignItems:"center", gap:"7px" }}>
            <div style={{ width:"9px", height:"9px", borderRadius:"50%", background:color, flexShrink:0 }} />
            <span style={{ fontSize:"15px", fontWeight:500, color:"#18100A", lineHeight:1.2 }}>{org.name}</span>
          </div>
          <button onClick={onClose} style={{ background:"transparent", border:"none", color:"#18100A", cursor:"pointer", fontSize:"20px", lineHeight:1, flexShrink:0, paddingLeft:"8px" }}>×</button>
        </div>
        <div style={{ fontSize:"11px", color, letterSpacing:"0.1em", textTransform:"uppercase", marginBottom: org.description || org.website ? "8px" : 0 }}>{org.stance}</div>
        {org.website && (
          <a href={org.website} target="_blank" rel="noopener noreferrer"
            style={{ fontSize:"13px", color:"#5B4FCE", textDecoration:"none", display:"inline-flex", alignItems:"center", gap:"3px", marginBottom:"6px" }}>
            ↗ {websiteDisplay}
          </a>
        )}
        {org.description && <p style={{ fontSize:"13px", color:"#6B5B78", fontStyle:"italic", lineHeight:1.5, margin:0 }}>{org.description}</p>}
      </div>

      <div style={{ flex:1, overflowY:"auto", padding:"12px 14px" }}>
        <div style={{ fontSize:"11px", color:"#6B5B78", letterSpacing:"0.12em", textTransform:"uppercase", marginBottom:"10px" }}>
          Linked Articles ({linkedItems.length})
        </div>
        {linkedItems.length === 0 && (
          <div style={{ fontSize:"13px", color:"rgba(122,128,104,0.5)", fontStyle:"italic" }}>No articles linked yet.</div>
        )}
        {linkedItems.map(item => (
          <button key={item.id} onClick={() => { onNavigate(item.id); onClose(); }}
            style={{ display:"block", width:"100%", textAlign:"left", background:"transparent", border:"none", borderBottom:"1px solid rgba(91,79,206,0.10)", padding:"7px 0", cursor:"pointer" }}>
            <div style={{ display:"flex", gap:"5px", alignItems:"center", marginBottom:"2px" }}>
              <span style={{ fontSize:"11px", color: COLOR[item.theme]||"#6B5B78", letterSpacing:"0.07em", textTransform:"uppercase", flexShrink:0 }}>{item.theme}</span>
              <span style={{ fontSize:"11px", color:"#6B5B78" }}>· {item.source}</span>
            </div>
            <div style={{ fontSize:"13px", color:"#18100A", lineHeight:1.4, fontFamily:"'Palatino Linotype',Palatino,serif" }}>
...
function GardenView({ pool, readItems, onToggleRead, notes, onSaveNote, publicMode, onRemove, paths = [], onSavePath, onDeletePath, orgs = [], orgLinks = [], onSaveOrgLink, onDeleteOrgLink }) {
    ...
    activePaths.forEach(p => {
      ...
      for (let i = 0; i < ids.length - 1; i++) {
        ...
        pg.append("line")
          ...
          .attr("stroke", p.color || "#5B4FCE").attr("stroke-width", 2.5)
      ...
      ids.forEach((id, idx) => {
        ...
        pg.append("circle").attr("cx", pos.x).attr("cy", pos.y).attr("r", 9)
          .attr("fill", p.color || "#5B4FCE").attr("fill-opacity", 0.88)
          .attr("pointer-events", "none");
  ...
  useEffect(() => {
    ...
    THEMES.forEach(t => {
      ...
      bg.append("text")
        ...
        .attr("font-family","'Palatino Linotype',Palatino,serif")
        .attr("stroke","#FEFEFD").attr("stroke-width", 4).attr("stroke-linejoin","round")
    ...
    const linkSel = g.append("g")
      ...
      .attr("stroke","rgba(91,79,206,0.10)").attr("stroke-width",0.8)
    ...
    function resetOp() {
      ...
      linkSel.attr("stroke","rgba(91,79,206,0.10)").attr("stroke-width",0.8);
    }
  ...
  const drawOrgOverlay = useCallback(() => {
    ...
    currentLinks.forEach(link => {
      ...
      if (!op || !ap) return;
      const color = STANCE_COLORS[currentOrgs.find(o => o.id === link.org_id)?.stance] || "#6B5B78";
      og.append("line")
  ...
  useEffect(() => {
    ...
    if(dimTheme) {
      ...
      nodeSelRef.current.attr("fill-opacity", d => readItems.has(d.url) ? 0.2 : 0.82);
      linkSelRef.current.attr("stroke","rgba(91,79,206,0.10)").attr("stroke-width",0.8);
    }
  }, [dimTheme, readItems]);
  ...
  return (
    <div style={{ position:"relative", width:"100%", height:"100%" }}>
      ...
      <div style={{ position:"absolute", top:"10px", left:"10px", zIndex:10, display:"flex", flexDirection:"column", gap:"3px" }}>
        {THEMES.map(t => (
          <button key={t.name} onClick={() => setDimTheme(d => d===t.name ? null : t.name)}
            style={{ ...F, display:"flex", alignItems:"center", gap:"6px", background:dimTheme===t.name?t.color+"22":"#F3EFFE", border:"1px solid "+(dimTheme===t.name?t.color:"rgba(91,79,206,0.20)"), borderRadius:"3px", padding:"3px 8px", cursor:"pointer", transition:"all 0.15s" }}>
            <div style={{ width:"6px", height:"6px", borderRadius:"50%", background:t.color, flexShrink:0 }} />
            <span style={{ fontSize:"12px", letterSpacing:"0.07em", textTransform:"uppercase", color:dimTheme===t.name?t.color:"#18100A", whiteSpace:"nowrap" }}>{t.name}</span>
          </button>
        ))}
        {dimTheme && (
          <button onClick={() => setDimTheme(null)} style={{ ...F, background:"#F3EFFE", border:"1px solid rgba(91,79,206,0.20)", borderRadius:"3px", padding:"3px 8px", cursor:"pointer", color:"#6B5B78", fontSize:"12px", letterSpacing:"0.07em", textTransform:"uppercase" }}>Clear</button>
        )}
        {publicMode ? (
          paths.length > 0 && (
            <button onClick={() => setTrailsOpen(v => !v)}
              style={{ ...F, marginTop:"6px", background: trailsOpen?"rgba(91,79,206,0.14)":"#F3EFFE", border:"1px solid "+(trailsOpen?"rgba(91,79,206,0.50)":"rgba(91,79,206,0.20)"), borderRadius:"3px", padding:"3px 8px", cursor:"pointer", transition:"all 0.15s" }}>
              <span style={{ fontSize:"12px", letterSpacing:"0.07em", textTransform:"uppercase", color: trailsOpen?"#5B4FCE":"#18100A", whiteSpace:"nowrap" }}>
                Trails ({paths.length})
              </span>
            </button>
          )
          ...
          <button onClick={() => setPathsPanelOpen(v => !v)}
            style={{ ...F, marginTop:"6px", background: pathsPanelOpen?"rgba(91,79,206,0.14)":"#F3EFFE", border:"1px solid "+(pathsPanelOpen?"rgba(91,79,206,0.50)":"rgba(91,79,206,0.20)"), borderRadius:"3px", padding:"3px 8px", cursor:"pointer", transition:"all 0.15s" }}>
            <span style={{ fontSize:"12px", letterSpacing:"0.07em", textTransform:"uppercase", color: pathsPanelOpen?"#5B4FCE":"#18100A", whiteSpace:"nowrap" }}>
              Trails{paths.length > 0 ? ` (${paths.length})` : ""}
            </span>
          </button>
        )}
        <button onClick={() => setShowOrgs(v => !v)}
          style={{ ...F, marginTop:"3px", background: showOrgs?"rgba(16,104,212,0.13)":"#F3EFFE", border:"1px solid "+(showOrgs?"rgba(16,104,212,0.5)":"rgba(91,79,206,0.20)"), borderRadius:"3px", padding:"3px 8px", cursor:"pointer", transition:"all 0.15s" }}>
          <span style={{ fontSize:"12px", letterSpacing:"0.07em", textTransform:"uppercase", color: showOrgs?"#5B4FCE":"#18100A", whiteSpace:"nowrap" }}>
      ...
      {publicMode && trailsOpen && paths.length > 0 && (
        <div style={{ ...F, position:"absolute", bottom:"12px", left:"10px", zIndex:10, background:"#F3EFFE", border:"1px solid rgba(91,79,206,0.15)", borderRadius:"4px", padding:"10px 12px" }}>
          <div style={{ fontSize:"11px", color:"#6B5B78", letterSpacing:"0.12em", textTransform:"uppercase", marginBottom:"7px" }}>Trails</div>
          {paths.map(p => (
            <div key={p.id} style={{ display:"flex", alignItems:"center", gap:"7px", marginBottom:"4px" }}>
              ...
              <span style={{ fontSize:"12px", color:"#18100A" }}>{p.name}</span>
              {p.description && <span style={{ fontSize:"11px", color:"#6B5B78", fontStyle:"italic" }}>— {p.description.length>36 ? p.description.slice(0,36)+"…" : p.description}</span>}
            </div>
          ))}
        </div>
      )}
      ...
      <div ref={searchRef} style={{ position:"absolute", top:"10px", right: (sideOpen||!!selectedOrg) ? "300px" : "10px", zIndex:20, transition:"right 0.2s" }}>
        <div style={{ display:"flex", alignItems:"center", background:"#F3EFFE", border:"1px solid rgba(91,79,206,0.20)", borderRadius:"3px", padding:"4px 8px", gap:"6px" }}>
          <span style={{ fontSize:"13px", color:"#6B5B78", opacity:0.6 }}>⌕</span>
          <input
            value={searchQuery}
            ...
            style={{ ...F, background:"transparent", border:"none", outline:"none", fontSize:"13px", color:"#18100A", width:"180px", "::placeholder":{ color:"#6B5B78" } }}
          />
          {searchQuery && <button onClick={() => { setSearchQuery(""); setSearchOpen(false); }} style={{ background:"none", border:"none", cursor:"pointer", color:"#6B5B78", fontSize:"14px", padding:0, lineHeight:1 }}>×</button>}
        </div>
        {searchOpen && searchResults.length > 0 && (
          <div style={{ position:"absolute", top:"calc(100% + 4px)", right:0, background:"#F3EFFE", border:"1px solid rgba(91,79,206,0.15)", borderRadius:"3px", minWidth:"280px", maxWidth:"360px", boxShadow:"0 4px 16px rgba(43,45,66,0.1)", overflow:"hidden" }}>
            {searchResults.map(r => {
              if (r._type === "org") {
                const color = STANCE_COLORS[r.stance] || "#6B5B78";
                return (
                  <button key={"org-"+r.id} onClick={() => { setSelectedOrg(r); setSearchOpen(false); setSearchQuery(""); }}
                    style={{ ...F, display:"block", width:"100%", textAlign:"left", background:"transparent", border:"none", borderBottom:"1px solid rgba(91,79,206,0.10)", padding:"8px 12px", cursor:"pointer", transition:"background 0.1s" }}
                    onMouseEnter={e => e.currentTarget.style.background="rgba(243,239,254,0.7)"}
                    onMouseLeave={e => e.currentTarget.style.background="transparent"}>
                    <div style={{ fontSize:"13px", color:"#18100A", lineHeight:1.3, marginBottom:"3px" }}>{r.name}</div>
                    <div style={{ display:"flex", gap:"6px", alignItems:"center" }}>
                      <span style={{ fontSize:"11px", color, letterSpacing:"0.07em", textTransform:"uppercase" }}>Org</span>
                      <span style={{ fontSize:"11px", color:"#6B5B78" }}>· {r.stance}</span>
                    </div>
                  </button>
                );
              }
                ...
                return (
                  <button key={"trail-"+r.id} onClick={() => { setHighlightedTrailId(r.id); if (publicMode) { setTrailsOpen(true); pathsVisibleRef.current = true; } else setPathsPanelOpen(true); setSearchOpen(false); setSearchQuery(""); }}
                    ...
                    <div style={{ display:"flex", alignItems:"center", gap:"8px", marginBottom:"2px" }}>
                      <svg width="16" height="4" style={{ flexShrink:0 }}><line x1="0" y1="2" x2="16" y2="2" stroke={r.color||"#5B4FCE"} strokeWidth="2" strokeDasharray="4 2" /></svg>
                      <span style={{ fontSize:"13px", color:"#18100A", lineHeight:1.3 }}>{r.name}</span>
                    </div>
                    <span style={{ fontSize:"11px", color:"#6B5B78", letterSpacing:"0.07em", textTransform:"uppercase" }}>Trail · {(r.item_ids||[]).length} stops</span>
                  </button>
                );
              }
              return (
                <button key={r.id} onClick={() => { navigateToNode(r.id); setSearchOpen(false); setSearchQuery(""); }}
                  ...
                  onMouseEnter={e => e.currentTarget.style.background="rgba(243,239,254,0.7)"}
                  onMouseLeave={e => e.currentTarget.style.background="transparent"}>
                  <div style={{ fontSize:"13px", color:"#18100A", lineHeight:1.3, marginBottom:"3px" }}>{r.title}</div>
                  <div style={{ display:"flex", gap:"6px", alignItems:"center" }}>
                    <span style={{ fontSize:"11px", color: COLOR[r.theme] || "#6B5B78", letterSpacing:"0.08em", textTransform:"uppercase" }}>{r.theme}</span>
                    <span style={{ fontSize:"11px", color:"#6B5B78" }}>· {r.source}</span>
                  </div>
                </button>
              );
            })}
        ...
        {searchOpen && searchQuery.trim() && searchResults.length === 0 && (
          <div style={{ position:"absolute", top:"calc(100% + 4px)", right:0, background:"#F3EFFE", border:"1px solid rgba(91,79,206,0.15)", borderRadius:"3px", padding:"10px 12px", fontSize:"13px", color:"#6B5B78", fontStyle:"italic" }}>No results</div>
        )}
      </div>
      ...
      {noteItem  && <NotesModal item={noteItem} notes={notes} onSave={onSaveNote} onClose={() => setNoteItem(null)} />}
      {!ready    && <div style={{ position:"absolute",inset:0,background:"#FEFEFD",display:"flex",alignItems:"center",justifyContent:"center",zIndex:99 }}><span style={{ ...F, fontSize:"13px",color:"#6B5B78",letterSpacing:"0.15em",textTransform:"uppercase" }}>Growing the garden…</span></div>}
    </div>
  );
}
...
function StatBar({ value, max, color }) {
  ...
  return (
    <div style={{ display:"flex", alignItems:"center", gap:"10px" }}>
      <div style={{ flex:1, height:"4px", background:"rgba(91,79,206,0.25)", borderRadius:"2px", overflow:"hidden" }}>
        <div style={{ height:"100%", width: pct+"%", background: color, borderRadius:"2px", transition:"width 0.6s ease" }} />
      </div>
      <span style={{ fontSize:"12px", color:"#18100A", width:"36px", textAlign:"right" }}>{value}/{max}</span>
    </div>
  );
}
    ...
    <div style={{ ...F, maxWidth:"680px", margin:"0 auto", padding:"32px 24px 80px" }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(130px, 1fr))", gap:"10px", marginBottom:"36px" }}>
        {[
          ...
          { label:"Essays read",   value: readEssays.length, sub:`of ${essays.length}`, color:"#5B4FCE" },
          ...
          { label:"Notes written", value: totalNotes, sub:`across ${pool.length} items`, color:"#5B4FCE" },
        ].map(s => (
          <div key={s.label} style={{ background:"rgba(243,239,254,0.7)", border:"1px solid rgba(91,79,206,0.25)", borderTop:"2px solid "+s.color, borderRadius:"3px", padding:"14px 14px 12px" }}>
            <div style={{ fontSize:"22px", color: s.color, fontWeight:400, lineHeight:1, marginBottom:"5px" }}>{s.value}</div>
            <div style={{ fontSize:"12px", color:"#18100A", letterSpacing:"0.05em" }}>{s.label}</div>
            <div style={{ fontSize:"12px", color:"#18100A", marginTop:"2px" }}>{s.sub}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ marginBottom:"36px" }}>
        <div style={{ fontSize:"12px", color:"#6B5B78", letterSpacing:"0.14em", textTransform:"uppercase", marginBottom:"14px" }}>Academic breakdown</div>
        <div style={{ display:"flex", flexDirection:"column", gap:"10px" }}>
          {[
            { label:"Foundational papers", read: readFound.length, total: foundational.length, color:"#5B4FCE" },
            { label:"Recent papers",       read: readRecent.length, total: recent.length,       color:"#7A9B6A" },
          ].map(row => (
            <div key={row.label}>
              <div style={{ display:"flex", justifyContent:"space-between", marginBottom:"5px" }}>
                <span style={{ fontSize:"13px", color:"#18100A" }}>{row.label}</span>
                <span style={{ fontSize:"13px", color: row.read===row.total && row.total>0 ? "#16A34A":"#18100A" }}>
      ...
      {/* Per-theme breakdown */}
      <div style={{ marginBottom:"36px" }}>
        <div style={{ fontSize:"12px", color:"#18100A", letterSpacing:"0.14em", textTransform:"uppercase", marginBottom:"14px" }}>Progress by theme</div>
            ...
            <div key={t.name}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"baseline", marginBottom:"5px" }}>
                <div style={{ display:"flex", alignItems:"center", gap:"7px" }}>
                  ...
                  <span style={{ fontSize:"13px", color:"#18100A" }}>{t.name}</span>
                  {t.noted > 0 && <span style={{ fontSize:"11px", color:"#5B4FCE", background:"rgba(91,79,206,0.10)", border:"1px solid rgba(91,79,206,0.15)", borderRadius:"2px", padding:"1px 5px" }}>✎ {t.noted} note{t.noted>1?"s":""}</span>}
                </div>
                <span style={{ fontSize:"12px", color: t.read===t.total ? "#16A34A":"#6B5B78" }}>
                  {t.read}/{t.total}
                </span>
              </div>
      ...
      {notedItems.length > 0 && (
        <div>
          <div style={{ fontSize:"12px", color:"#18100A", letterSpacing:"0.14em", textTransform:"uppercase", marginBottom:"14px" }}>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:"2px" }}>
            {notedItems.map(item => {
              ...
              const c = COLOR[item.theme] || "#6B5B78";
              return (
                <div key={item.url} style={{ background:"rgba(243,239,254,0.7)", border:"1px solid rgba(91,79,206,0.20)", borderLeft:"3px solid "+c, borderRadius:"3px", padding:"12px 14px" }}>
                  <div style={{ display:"flex", gap:"6px", alignItems:"center", marginBottom:"6px", flexWrap:"wrap" }}>
                    <span style={{ fontSize:"11px", letterSpacing:"0.1em", textTransform:"uppercase", padding:"1px 6px", borderRadius:"2px", background:c+"22", color:c, fontWeight:600 }}>{item.theme}</span>
                    <span style={{ fontSize:"12px", color:"#18100A", fontStyle:"italic" }}>{item.source} · {item.published}</span>
                  </div>
                  <div style={{ fontSize:"14px", color:"#18100A", marginBottom:"10px", lineHeight:1.4, fontWeight:500 }}>{item.title}</div>
                  {n.quote && (
                    <div style={{ marginBottom:"8px", borderLeft:"2px solid rgba(91,79,206,0.30)", paddingLeft:"8px" }}>
                      <div style={{ fontSize:"11px", color:"#5B4FCE", letterSpacing:"0.1em", textTransform:"uppercase", marginBottom:"3px" }}>Quote</div>
                      <div style={{ fontSize:"13px", color:"#18100A", lineHeight:1.6, fontStyle:"italic" }}>{n.quote}</div>
                    </div>
                  )}
                  {n.argument && (
                    <div style={{ marginBottom: n.thoughts ? "8px" : 0 }}>
                      <div style={{ fontSize:"11px", color:"#5B4FCE", letterSpacing:"0.1em", textTransform:"uppercase", marginBottom:"3px" }}>Argument</div>
                      <div style={{ fontSize:"13px", color:"#18100A", lineHeight:1.6 }}>{n.argument}</div>
                    </div>
                  )}
                  {n.thoughts && (
                    <div>
                      <div style={{ fontSize:"11px", color:"#7A9B6A", letterSpacing:"0.1em", textTransform:"uppercase", marginBottom:"3px" }}>My thoughts</div>
                      <div style={{ fontSize:"13px", color:"#6B5B78", lineHeight:1.6 }}>{n.thoughts}</div>
                    </div>
                  )}
                </div>
              );
      ...
      {totalRead === 0 && (
        <div style={{ marginTop:"60px", textAlign:"center" }}>
          <div style={{ fontSize:"14px", color:"#18100A", fontStyle:"italic" }}>No items marked as read yet.</div>
          <div style={{ fontSize:"13px", color:"#6B5B78", marginTop:"6px" }}>Head to The Plot tab and start reading.</div>
        </div>
      )}
    </div>
  );
...
const NAV = { ...F, background:"transparent", border:"1px solid rgba(91,79,206,0.25)", color:"#6B5B78", width:"28px", height:"28px", borderRadius:"3px", cursor:"pointer", fontSize:"15px", display:"inline-flex", alignItems:"center", justifyContent:"center" };
...
export function PublicGardenPage() {
  ...
  if (!loaded) return <div style={{ ...F, height:"100vh", background:"#FEFEFD", display:"flex", alignItems:"center", justifyContent:"center" }}><span style={{ fontSize:"13px", color:"#6B5B78", letterSpacing:"0.15em", textTransform:"uppercase" }}>Growing the garden…</span></div>;
  ...
  return (
    <div style={{ height:"100vh", background:"#FEFEFD", color:"#18100A", display:"flex", flexDirection:"column", overflow:"hidden" }}>
      {/* Header */}
      <div style={{ background:"#F3EFFE", borderBottom:"1px solid rgba(91,79,206,0.15)", paddingLeft:"18px", paddingRight:"18px", paddingTop:"10px", paddingBottom:"10px", flexShrink:0, backdropFilter:"blur(4px)", zIndex:100 }}>
        <div style={{ display:"flex", alignItems:"center", gap:"8px", marginBottom:"6px" }}>
          <span style={{ fontSize:"16px", lineHeight:1 }}>🪸</span>
          <span style={{ ...F, fontSize:"13px", color:"#18100A", letterSpacing:"0.12em", textTransform:"uppercase" }}>Plot Twists</span>
          {lastUpdated && <span style={{ ...F, fontSize:"12px", color:"#6B5B78", fontStyle:"italic", marginLeft:"auto" }}>Updated {lastUpdated}</span>}
        </div>
        <p style={{ ...F, fontSize:"13px", color:"#18100A", lineHeight:1.5, margin:"0 0 8px" }}>
          ...
          <a href="https://prabhnoorkohli.fyi" target="_blank" rel="noopener noreferrer" style={{ color:"#5B4FCE", textDecoration:"underline" }}>More about me & my work ↗</a>
        </p>
        {/* Tab bar */}
        <div style={{ display:"flex", gap:"4px", marginBottom:"6px" }}>
          {PUBLIC_TABS.map(t => (
            <button key={t.id} onClick={() => setTab(t.id)}
              style={{ ...F, background: tab===t.id ? "rgba(91,79,206,0.08)" : "transparent", border: tab===t.id ? "1px solid rgba(91,79,206,0.20)" : "1px solid transparent", borderRadius:"3px", padding:"4px 12px", fontSize:"12px", letterSpacing:"0.09em", textTransform:"uppercase", color: tab===t.id ? "#5B4FCE" : "#6B5B78", cursor:"pointer", gap:"5px" }}>
              <span style={{ marginRight:"4px" }}>{t.icon}</span>{t.label}
            </button>
          ))}
        </div>
        {/* Tab description */}
        <p style={{ ...F, fontSize:"12px", color:"#6B5B78", fontStyle:"italic", lineHeight:1.4, margin:0 }}>
...
function SetPasswordScreen({ onDone }) {
  ...
  return (
    <div style={{ ...F, height:"100vh", background:"#FEFEFD", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:"32px" }}>
      <div style={{ textAlign:"center" }}>
        <div style={{ fontSize:"14px", letterSpacing:"0.18em", textTransform:"uppercase", color:"#18100A" }}>Plot Twists</div>
        <div style={{ fontSize:"13px", color:"#6B5B78", fontStyle:"italic", marginTop:"6px" }}>Set your password</div>
      </div>
      {done ? (
        <div style={{ fontSize:"13px", color:"#18100A" }}>Password set. Entering…</div>
      ) : (
        <form onSubmit={handleSubmit} style={{ display:"flex", flexDirection:"column", gap:"12px", width:"260px" }}>
          <input type="password" required placeholder="New password" value={password} onChange={e => setPassword(e.target.value)}
            style={{ ...F, background:"rgba(243,239,254,0.8)", border:"1px solid rgba(91,79,206,0.20)", color:"#18100A", fontSize:"13px", padding:"9px 12px", borderRadius:"3px", outline:"none" }} />
          <input type="password" required placeholder="Confirm password" value={confirm} onChange={e => setConfirm(e.target.value)}
            style={{ ...F, background:"rgba(243,239,254,0.8)", border:"1px solid rgba(91,79,206,0.20)", color:"#18100A", fontSize:"13px", padding:"9px 12px", borderRadius:"3px", outline:"none" }} />
          {error && <div style={{ fontSize:"13px", color:"#5B4FCE" }}>{error}</div>}
          <button type="submit" disabled={loading}
            style={{ ...F, background:"#5B4FCE", color:"#FFFFFF", border:"none", padding:"10px", fontSize:"13px", letterSpacing:"0.12em", textTransform:"uppercase", cursor: loading ? "default" : "pointer", borderRadius:"3px", opacity: loading ? 0.6 : 1 }}>
...
function LoginScreen() {
  ...
  return (
    <div style={{ ...F, height:"100vh", background:"#FEFEFD", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:"32px" }}>
      <div style={{ textAlign:"center" }}>
        <div style={{ fontSize:"14px", letterSpacing:"0.18em", textTransform:"uppercase", color:"#18100A" }}>Plot Twists</div>
        <div style={{ fontSize:"13px", color:"#6B5B78", fontStyle:"italic", marginTop:"6px" }}>Prabhnoor's Digital Garden</div>
      </div>
      <form onSubmit={handleSubmit} style={{ display:"flex", flexDirection:"column", gap:"12px", width:"260px" }}>
        <input
          type="password"
...
export default function App() {
  ...
  if (!authReady) return <div style={{ ...F, height:"100vh", background:"#FEFEFD", display:"flex", alignItems:"center", justifyContent:"center" }}><span style={{ fontSize:"13px", color:"#6B5B78", letterSpacing:"0.15em", textTransform:"uppercase" }}>Loading…</span></div>;
  if (recovering)  return <SetPasswordScreen onDone={() => setRecovering(false)} />;
  if (!user) return <LoginScreen />;
  if (!loaded) return <div style={{ ...F, height:"100vh", background:"#FEFEFD", display:"flex", alignItems:"center", justifyContent:"center" }}><span style={{ fontSize:"13px", color:"#6B5B78", letterSpacing:"0.15em", textTransform:"uppercase" }}>Loading…</span></div>;

  return (
    <div style={{ height:"100vh", background:"#FEFEFD", color:"#18100A", display:"flex", flexDirection:"column", overflow:"hidden" }}>
      {/* Top bar */}
      <div style={{ height:"44px", background:"#F3EFFE", borderBottom:"1px solid rgba(91,79,206,0.15)", display:"flex", alignItems:"center", paddingLeft:"18px", paddingRight:"18px", flexShrink:0, gap:"16px", backdropFilter:"blur(4px)", zIndex:100 }}>
        <div style={{ display:"flex", alignItems:"center", gap:"8px" }}>
          <div style={{ width:"6px", height:"6px", borderRadius:"50%", background:"#5B4FCE" }} />
          <span style={{ ...F, fontSize:"13px", color:"#18100A", letterSpacing:"0.12em", textTransform:"uppercase" }}>Plot Twists</span>
        </div>
        {!isMobile && (
          <>
            <div style={{ height:"100%", borderLeft:"1px solid rgba(91,79,206,0.25)", marginLeft:"4px" }} />
            <TabBar active={tab} onChange={setTab} />
          </>
        )}
        <div ref={topbarPanelRef} style={{ marginLeft:"auto", display:"flex", gap:"14px", alignItems:"center", position:"relative" }}>
          {!isMobile && totalRead > 0  && <button onClick={() => setTopbarPanel(v => v==="read"  ? null : "read")}  style={{ ...F, background:"none", border:"none", cursor:"pointer", fontSize:"13px", color: topbarPanel==="read"  ?"#5B4FCE":"#6B5B78", textDecoration: topbarPanel==="read"  ?"underline":"none", padding:0 }}>{totalRead} read</button>}
          {!isMobile && totalNotes > 0 && <button onClick={() => setTopbarPanel(v => v==="notes" ? null : "notes")} style={{ ...F, background:"none", border:"none", cursor:"pointer", fontSize:"13px", color: topbarPanel==="notes" ?"#DB2777":"#6B5B78", textDecoration: topbarPanel==="notes" ?"underline":"none", padding:0 }}>{totalNotes} notes</button>}
          {!isMobile && customItems.length > 0 && <span style={{ fontSize:"13px", color:"#6B5B7888" }}>+{customItems.length} custom</span>}
          <button onClick={() => supabase.auth.signOut()} style={{ ...NAV, fontSize:"12px", width:"auto", padding:"0 8px", letterSpacing:"0.06em" }}>Sign out</button>
          {topbarPanel && (() => {
            ...
            return (
              <div style={{ position:"absolute", top:"calc(100% + 10px)", right:0, width:"340px", maxHeight:"420px", overflowY:"auto", background:"#F3EFFE", border:"1px solid rgba(91,79,206,0.25)", borderRadius:"4px", boxShadow:"0 4px 18px rgba(43,45,66,0.08)", zIndex:200 }}>
                <div style={{ padding:"10px 14px 8px", borderBottom:"1px solid rgba(91,79,206,0.08)", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                  <span style={{ fontSize:"11px", color:"#6B5B78", letterSpacing:"0.12em", textTransform:"uppercase" }}>{topbarPanel === "read" ? "Read" : "Notes"} · {items.length} item{items.length!==1?"s":""}</span>
                  <button onClick={() => setTopbarPanel(null)} style={{ background:"none", border:"none", cursor:"pointer", color:"#6B5B78", fontSize:"16px", lineHeight:1, padding:0 }}>×</button>
                </div>
                {items.length === 0 && <div style={{ padding:"18px 14px", fontSize:"13px", color:"#6B5B78", fontStyle:"italic" }}>Nothing here yet.</div>}
                {items.map(item => {
                  const note = notes[item.url];
                  return (
                    <div key={item.url} style={{ padding:"9px 14px", borderBottom:"1px solid rgba(91,79,206,0.10)" }}>
                      <a href={item.url} target="_blank" rel="noopener noreferrer" style={{ fontSize:"13px", color:"#18100A", textDecoration:"none", display:"block", lineHeight:1.4, marginBottom:"3px", fontWeight:500 }}>{item.title} ↗</a>
                      <div style={{ display:"flex", gap:"8px", alignItems:"center", flexWrap:"wrap" }}>
                        <span style={{ fontSize:"12px", color: COLOR[item.theme]||"#6B5B78", letterSpacing:"0.07em", textTransform:"uppercase" }}>{item.theme}</span>
                        <span style={{ fontSize:"12px", color:"#6B5B78", fontStyle:"italic" }}>{item.source}</span>
                      </div>
                      {topbarPanel === "notes" && note?.argument && <div style={{ fontSize:"13px", color:"#18100A", lineHeight:1.5, marginTop:"5px", borderLeft:"2px solid rgba(91,79,206,0.30)", paddingLeft:"7px" }}>{note.argument.slice(0,120)}{note.argument.length>120?"…":""}</div>}
                    </div>
                  );
                })}
              </div>
